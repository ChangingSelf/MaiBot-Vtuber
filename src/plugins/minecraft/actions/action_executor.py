...
class MinecraftActionExecutor:
    ...
    async def execute_maicore_action(
        self, mland: mineland.MineLand, message_json_str: str, logger
        ...
        if not self.game_state.is_ready_for_next_action():
            ...
            await self._wait_for_action_completion(mland, logger)
        ...
        try:
            next_obs, next_code_info, next_event, next_done, next_task_info = execute_mineland_action(
                mland=mland, current_actions=current_actions
            )
            ...
            if self.game_state.get_effective_done():
                ...
                self._reset_environment(mland, logger)
    ...
    async def execute_no_op(self, mland: mineland.MineLand, logger):
        """执行no_op操作并更新状态"""
        try:
            no_op_actions = mineland.Action.no_op(1)  # 固定为单智能体
            next_obs, next_code_info, next_event, next_done, next_task_info = execute_mineland_action(
                mland=mland, current_actions=no_op_actions
            )
    ...
    async def _wait_for_action_completion(self, mland: mineland.MineLand, logger):
        ...
        while not self.game_state.is_ready_for_next_action() and wait_cycles < self.max_wait_cycles:
            ...
            await self.execute_no_op(mland, logger)
            wait_cycles += 1
    ...
    def _reset_environment(self, mland: mineland.MineLand, logger):
        """重置环境"""
        initial_obs = mland.reset()
