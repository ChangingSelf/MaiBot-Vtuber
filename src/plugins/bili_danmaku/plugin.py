...
from maim_message import MessageBase, UserInfo, BaseMessageInfo, GroupInfo, FormatInfo, Seg # 假设可以这样导入
# 尝试导入 aiohttp
try:
    import aiohttp
except ImportError:
...
class BiliDanmakuPlugin(BasePlugin):
    ...
    def __init__(self, core: VupNextCore, plugin_config: Dict[str, Any]):
        ...
        if aiohttp is None:
            self.logger.error("aiohttp library not found. Please install it (`pip install aiohttp`). BiliDanmakuPlugin disabled.")
            self.enabled = False
            return
        ...
        try:
            ...
            async with self._session.get(self.api_url, timeout=10) as response:
                ...
                if data.get("code") == 0:
                    ...
                    self.logger.warning(f"Bilibili API 返回错误: code={data.get('code')}, message={data.get('message')}")
