...
class ScreenMonitorPlugin(BasePlugin):
    ...
    def __init__(self, core: VupNextCore, plugin_config: Dict[str, Any]):
        ...
        if mss is None or openai is None or Image is None:
             ...
             logger.error(f"缺少必要的库: {', '.join(missing)}。请运行 `pip install mss openai Pillow`。ScreenMonitorPlugin 已禁用。")
             self.enabled = False
             return
        ...
        if not self.base_url:
            logger.error("OpenAI 兼容 Base URL (openai_compatible_base_url) 未在 config.toml 中配置！ScreenMonitorPlugin 已禁用。")
            self.enabled = False
            return
    ...
    async def setup(self):
        ...
        if prompt_ctx_service:
            ...
            logger.info(f"已向 PromptContext 注册动态屏幕上下文提供者 '{self.context_provider_name}' (优先级: {self.context_priority})。")
        else:
    ...
    async def _capture_and_process_screenshot(self):
        """执行截图、编码和调用 VL 模型。"""
        if not self.openai_client: return # 检查 OpenAI 客户端
