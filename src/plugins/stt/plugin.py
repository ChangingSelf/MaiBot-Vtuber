...
class STTPlugin(BasePlugin):
    ...
    async def setup(self):
        ...
        self.logger.info("启动 STT 音频监听和处理任务..." + (f" (设备: {self.input_device_index})" if self.input_device_index is not None else " (默认设备)"))
    ...
    async def transcribe_stream(self) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            self.logger.info("开始 STT 音频流..." + (f" (设备索引: {self.input_device_index})" if self.input_device_index is not None else " (默认设备)"))
            ...
            stream.start()
            self.logger.info(f"音频流已启动。监听语音 (VAD 阈值: {self.vad_threshold}, 静音: {self.min_silence_duration_ms}ms)...")
                        ...
                        if not triggered:
                            ...
                            if ws is None or ws.closed:
                                ...
                                try:
                                    ...
                                    receiver_task = asyncio.create_task(self._iflytek_receiver(ws, result_future), name="iFlytekReceiver")
                                    ...
                                    if audio_buffer:
                                        ...
                                        self.logger.debug(f"在首帧中发送了 {len(audio_buffer)} 个缓冲块 ({len(audio_bytes)} bytes)。")
                                        audio_buffer.clear() 
                                    ...
                                    if ws and not ws.closed: await ws.close(); ws = None
                            ...
                            if ws and not ws.closed:
                                try:
                                    ...
                                    await self._close_iflytek_connection(ws, receiver_task, "send_error", close_session=False)
                                    ws = None; receiver_task = None; result_future = None
                                    triggered = False
                                    continue
                    else: 
                        if triggered:
                            ...
                            if silence_blocks >= silence_frames_needed:
                                self.logger.info(f"VAD 未触发 (静音 {silence_blocks * self.block_size_ms}ms 在 {speech_blocks} 语音块后达到)")
                                triggered = False
                                if speech_blocks >= min_speech_blocks:
                                     self.logger.info(f"语音段足够长 ({speech_blocks * self.block_size_ms}ms)，结束讯飞段落。")
                                     yield await self._end_iflytek_segment(ws, receiver_task, result_future, "silence_end", session)
                                else:
                                     self.logger.info(f"语音段太短 ({speech_blocks * self.block_size_ms}ms)，丢弃。")
                                     await self._close_iflytek_connection(ws, receiver_task, "short_speech", close_session=False)
                                ws = None; receiver_task = None; result_future = None
                                audio_buffer.clear() 
    ...
    def _build_iflytek_auth_url(self) -> str:
        ...
        signature_sha = hmac.new(cfg['api_secret'].encode('utf-8'), signature_origin.encode('utf-8'), digestmod=hashlib.sha256).digest()
                ...
                if msg.type == aiohttp.WSMsgType.TEXT:
                    try:
                        ...
                        if resp.get("code", -1) != 0:
                            ...
                            if not result_future.done(): result_future.set_exception(RuntimeError(err_msg))
                            break
                        data = resp.get("data", {}) 
    ...
    async def _end_iflytek_segment(self, ws: Optional[aiohttp.ClientWebSocketResponse],
                                 receiver_task: Optional[asyncio.Task],
                                 result_future: Optional[asyncio.Future],
                                 reason: str,
                                 session: Optional[aiohttp.ClientSession]) -> str:
    ...
    async def _close_iflytek_connection(self, ws: Optional[aiohttp.ClientWebSocketResponse],
                                      receiver_task: Optional[asyncio.Task], reason: str,
