...
def load_plugin_config() -> Dict[str, Any]:
    ...
    try:
        with open(config_path, 'rb') as f:
            if hasattr(tomllib, 'load'): return tomllib.load(f)
            else: 
                try: 
                    import toml; 
                    with open(config_path, 'r', encoding='utf-8') as rf: return toml.load(rf)
                except ImportError: logging.error("toml package needed for Python < 3.11."); return {}
                except FileNotFoundError: logging.warning(f"Config file not found: {config_path}"); return {}
    except Exception as e: logging.error(f"Error loading config: {config_path}: {e}", exc_info=True); return {}
...
class VTubeStudioPlugin(BasePlugin):
    ...
    def __init__(self, core: VupNextCore, plugin_config: Dict[str, Any]):
        ...
        # --- 依赖检查 ---
        if pyvts is None:
            self.logger.error("pyvts library not found. Please install it (`pip install pyvts`). VTubeStudioPlugin disabled.")
            self.enabled = False
            return
        ...
        # --- 加载配置 ---
        self.plugin_name = self.config.get('plugin_name', 'VUP_NEXT_VTS_Connector')
        self.developer = self.config.get('developer', 'VUP-NEXT User')
        ...
        try:
            ...
            if authenticated:
             ...
             if isinstance(e, KeyError) and "authenticated" in str(e):
                 self.logger.error(f"KeyError accessing 'authenticated' during authentication. Unexpected VTS response? {e}", exc_info=True)
             else:
    ...
    async def _register_hotkeys_context(self):
        ...
        try:
            ...
            instructional_text = "对于每一次回复，请在回复文本末尾添加指令标记来表达你现在的情感。对于日常聊天，进行情感与动作的表现是被鼓励的，通过热键id可以看出该热键对应的情感。格式为 `%{vts_trigger_hotkey:热键ID}%`。"
            hotkey_list_text = ""
    ...
    async def trigger_hotkey(self, hotkey_id: str) -> bool:
        ...
        Args:
            hotkey_id: The ID of the hotkey to trigger.
        ...
        try:
            ...
            if response and response.get("messageType") == "APIError":
                 ...
                 self.logger.error(f"API Error triggering hotkey '{hotkey_id}': ID {error_data.get('errorID')}, Msg: {error_data.get('message')}")
                 return False
                 ...
                 self.logger.warning(f"Unexpected response type when triggering hotkey '{hotkey_id}': {response.get('messageType') if response else 'No Response'}")
                 return False
