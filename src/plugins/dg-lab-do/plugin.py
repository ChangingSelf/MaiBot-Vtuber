...
class ElectricityMonitorPlugin(BasePlugin):
    ...
    def __init__(self, core: AmaidesuCore, plugin_config: Dict[str, Any]):  # TODO: 确认 core 类型
        ...
        # self.logger.info( # 此日志可移除，基类有通用初始化日志
    ...
    async def setup(self):
        ...
        self.logger.info("已注册通配符消息处理器 check_and_control_device。")  # logger 已是 self.logger
...
"""
                ...
                try:  # 添加 try-except 块以防 register_context_provider 本身出错
                    ...
                    self.logger.info(  # logger 已是 self.logger
                        f"已向 PromptContext 注册警告信息 '{self.context_provider_name}' (优先级: {effective_priority})。"
                    )
                    return  # 注册成功，任务完成
                except Exception as e:
                    self.logger.error(f"注册上下文时发生错误: {e}", exc_info=True)  # logger 已是 self.logger
                    # 即使出错也跳出循环，避免无限重试
                    return
            else:
                attempt += 1
                if attempt < max_retries:
                    self.logger.warning(  # logger 已是 self.logger
                        f"未找到 PromptContext 服务 (尝试 {attempt}/{max_retries})，将在 {retry_delay} 秒后重试..."
                    )
                    await asyncio.sleep(retry_delay)
                    retry_delay *= 2  # 可以增加重试间隔
                else:
                    self.logger.error(f"超过最大重试次数，未能注册电击警告上下文。")  # logger 已是 self.logger
                    return  # 达到最大次数，任务结束

    async def cleanup(self):
        self.logger.info("正在清理 ElectricityMonitorPlugin...")  # logger 已是 self.logger
        # 关闭 aiohttp Session
        if self.http_session:
            await self.http_session.close()
            self.logger.info("aiohttp.ClientSession 已关闭。")  # logger 已是 self.logger
        ...
        if prompt_ctx_service:
            try:
                ...
                self.logger.info(
                    f"已从 PromptContext 取消注册警告信息 '{self.context_provider_name}'。"
                )  # logger 已是 self.logger
            except Exception as e:  # 可能 provider 没注册成功，unregister 会报错
                self.logger.warning(
                    f"尝试取消注册 '{self.context_provider_name}' 时出错 (可能未成功注册): {e}"
                )  # logger 已是 self.logger
        ...
        await super().cleanup()
        self.logger.info("ElectricityMonitorPlugin 清理完成。")  # logger 已是 self.logger
    ...
    async def check_and_control_device(self, message: MessageBase):
        ...
        if (
            ...
            if self.keyword in text_content:
                self.logger.info(
                    f"检测到关键词 '{self.keyword}' 在消息中: '{text_content[:50]}...'"
                )  # logger 已是 self.logger
                ...
                if not self.control_lock.locked():
                    ...
                    self.logger.debug("控制锁已被占用，跳过此次触发以防止请求堆积。")  # logger 已是 self.logger
    ...
    async def _send_control_commands(self, original_text: str):
        ...
        # --- 准备初始 API 调用任务 ---
        self.logger.info(f"检测到 '{self.keyword}'，准备发送初始设置命令...")  # logger 已是 self.logger
        # 设置通道 A 强度
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                self.logger.error(f"初始命令 '{desc}' 执行失败: {result}")
        ...
        if initial_success_count < 4:
            self.logger.warning(
                f"初始控制命令发送完成，但有 {4 - initial_success_count} 个失败。仍将尝试在延迟后归零强度。"
            )
        else:
            self.logger.info(f"所有 4 个初始控制命令已成功发送。")

        # --- 等待并发送强度归零命令 ---
        self.logger.info("等待 2 秒后将强度归零...")
        await asyncio.sleep(2)

        reset_tasks = []
        self.logger.info("准备发送强度归零命令...")
        # 设置通道 A 强度为 0
        reset_tasks.append(
        ...
        for i, result in enumerate(reset_results):
            ...
            if isinstance(result, Exception):
                self.logger.error(f"强度归零命令 '{desc}' 执行失败: {result}")
        ...
        if reset_success_count == 2:
            self.logger.info("强度归零命令已成功发送。")
        else:
            self.logger.warning(f"强度归零命令发送完成，但有 {2 - reset_success_count} 个失败。")
    ...
    async def _make_api_call(self, url: str, payload: Dict, headers: Dict, description: str) -> bool:
        ...
        try:
            self.logger.debug(f"发送请求: {description} 到 {url}，数据: {payload}")
            async with self.http_session.post(url, json=payload, headers=headers) as response:
                if 200 <= response.status < 300:
                    resp_json = await response.json()
                    self.logger.info(f"成功: {description} (响应: {resp_json})")
                    return True
                else:
                    error_text = await response.text()
                    self.logger.error(f"失败: {description} (状态码: {response.status}, 错误: {error_text[:200]})")
                    return False
        except aiohttp.ClientError as e:
            self.logger.error(f"请求错误 ({description}): {e}")
            return False
        except asyncio.TimeoutError:
            self.logger.error(f"请求超时 ({description})")
            return False
        except Exception as e:
            self.logger.error(f"发送 API 请求时发生意外错误 ({description}): {e}", exc_info=True)
            return False
