    ...
    try:
        with open(config_path, 'rb') as f:
            if hasattr(tomllib, 'load'): return tomllib.load(f)
                ...
                except ImportError: logger.error("toml package needed for Python < 3.11."); return {}
                except FileNotFoundError: logger.warning(f"Config file not found: {config_path}"); return {}
    except Exception as e: logger.error(f"Error loading config: {config_path}: {e}", exc_info=True); return {}
...
class ElectricityMonitorPlugin(BasePlugin):
    ...
    def __init__(self, core: VupNextCore, plugin_config: Dict[str, Any]):
        ...
        # --- 检查依赖 ---
        if aiohttp is None:
             logger.error("aiohttp library not found. Please install it (`pip install aiohttp`). ElectricityMonitorPlugin disabled.")
             self.enabled = False
             return
        ...
        logger.info(f"ElectricityMonitorPlugin 初始化完成。将监听关键词 '{self.keyword}' 并控制 API: {self.api_base_url}")
...
"""
                 ...
                 try: # 添加 try-except 块以防 register_context_provider 本身出错
                     ...
                     logger.info(f"已向 PromptContext 注册警告信息 '{self.context_provider_name}' (优先级: {effective_priority})。")
                     return # 注册成功，任务完成
                 ...
                 if attempt < max_retries:
                      logger.warning(f"未找到 PromptContext 服务 (尝试 {attempt}/{max_retries})，将在 {retry_delay} 秒后重试...")
    ...
    async def check_and_control_device(self, message: MessageBase):
        ...
        # 检查消息类型和内容
        if (message.message_segment and 
            message.message_segment.type == "text" and 
            isinstance(message.message_segment.data, str)):
    ...
    async def _send_control_commands(self, original_text: str):
        ...
        # 设置通道 A 强度
        initial_tasks.append(self._make_api_call(
            strength_url, 
        ...
        # 设置通道 A 强度为 0
        reset_tasks.append(self._make_api_call(
            strength_url, 
