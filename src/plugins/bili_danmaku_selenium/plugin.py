        ...
        try:
            while not self.stop_event.is_set():
                try:
                    ...
                    await self._fetch_and_process_messages() #处理弹幕消息
            ...
            try:
                ...
                for element in danmaku_elements[-pre_max:]:  # 只处理最新的几条
                    try:
                        ...
                        gift = DanmakuMessage(
                            username=username,
                            text=f"【重要提示，验证码:】你收到{gift_count}个名称为“{gift_name}”的礼物，请说一段不超过20字祝福语来感谢ta，感谢语以“谢谢{username}送的{gift_name}，祝你”为开头", #这里可以加个验证，防止大模型被弹幕骗了，把验证放到麦麦主prompt那里
