                ...
                if service_instance:
                    if hasattr(service_instance, method_name):
                        ...
                        if asyncio.iscoroutinefunction(method_to_call):
                            ...
                            self.logger.info(f"通过服务 '{service_name}'.{method_name} 执行指令 '{command_name}' (参数: {args})。")
                            # 异步执行命令，不等待
                            execution_tasks.append(asyncio.create_task(method_to_call(*args))) 
                        else:
                            self.logger.warning(f"服务 '{service_name}' 上的方法 '{method_name}' 不是异步函数。无法执行指令 '{command_name}'。")
                    else:
                         self.logger.warning(f"找到服务 '{service_name}'，但未找到指令 '{command_name}' 所需的方法 '{method_name}'。")
                else:
